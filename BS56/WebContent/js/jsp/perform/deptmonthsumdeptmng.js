...
jQuery(function($){
	...
	$('#dataTabel').datagrid({
		...
		//货主默认为长沙11430101
		nowrap:false,
		url:baseURL+"/perform/deptmonthsum/getDeptmonthsumPageList.json?ctype=20", //数据来源
		sortName: 'id', //排序的列
		sortOrder: 'desc', //倒序
 ...
 */
function openCreate(){
	parent.addTab('绩效新增',baseURL+"/perform/deptmonthsum/todeptmonthsumdeptmngadd",'icon-mygroup');
}
...
function edit(){
	...
	parent.addTab('绩效修改',baseURL+"/perform/deptmonthsum/todeptmonthsumdeptmngedit?obid="+id,'icon-mygroup');
...
function doAuditTwo(){
	...
	var status = row.status;
	if(status != "20"){
		$.messager.alert('提示',"对不起，该信息还未提交,暂不能总评",'info');
		return;
	}
	parent.addTab('总经理评',baseURL+"/perform/deptmonthsum/todeptmonthsumdeptmngaudittwo?obid="+id,'icon-mygroup');
	...
	//出库单明细列表
	  ...
	  $('#worklist').datagrid({
	  		...
	  		columns:[[
	  			...
	  			{field:'createname',title:'创建人',width:$(this).width()*0.2,
