...
jQuery(function($){
	...
	$('#dataTable').datagrid({
		...
		url:baseURL+"/cost/vehiclerepairapply/getVehicleRepairPageList.json", //数据来源
		sortName: 'applydate', //排序的列
		sortOrder: 'desc', //倒序
		...
		columns:[[
			...
			{field:'routecode',title:'车组',width:5,
				formatter:function(value,row,index){return row.routecode;} //需要formatter一下才能显示正确的数据
			},
			{field:'vehicleno',title:'车牌',width:8,
				formatter:function(value,row,index){return row.vehicleno;}
			},
			{field:'applydate',title:'申报日期',width:12,
				formatter:function(value,row,index){return row.applydate;} //需要formatter一下才能显示正确的数据
			},
			{field:'vehiclemainitem',title:'维修内容',width:30,
				formatter:function(value,row,index){
					var status=row.status;
					if(status=='20')return row.vehiclemainitem;
					return row.mainitem;
				}
			},
			{field:'repairinfo',title:'维修类型',width:20,
				formatter:function(value,row,index){return row.repairinfo;}
			},
			{field:'totalcost',title:'维修金额',width:5,
				formatter:function(value,row,index){return row.totalcost;}
			},
			{field:'statusname',title:'审核状态',width:10,
				formatter:function(value,row,index){return row.statusname;}
			},
			{field:'isreply',title:'是否回单',width:5,
				formatter:function(value,row,index){
					var isreply=row.isreply;
					if(isreply=="10")return "否";
					else return "是";
				}
			},
			{field:'createname',title:'记录人',width:5,
