 ...
 */
   ...
   */
    function openAudit(){
	  ...
	  $('#checktime1').datebox('setValue',getDateYMD());
  	  var rows = $('#dataTabel').datagrid('getSelections');
  		if(rows.length==0){
...
//  		$('#checktime1').textbox("setText",nowTime);
  ...
  //清空卷烟品牌查询条件
    ...
    function doAudit(checkflag){
    	...
    	$.messager.confirm('提示',alertMsg,function(result){
    		if(result){
    			$('#add-fm').form('submit',{
        			url:baseURL+"/wms/cigarettedamaged/doAuditabnormal.json?inboundid="+abid+"&checkflag="+checkflag+"&checkdescribe="+checkdescribe+"&damagedtype="+damagedtype, 
  ...
  //删除
	function deleterow(){
		var rows = $('#dataTabel').datagrid('getSelections');
		if(rows.length==0){
			$.messager.alert('提示',"请选择你要删除的抽检信息",'info');
			return;
		}
		if(rows.length > 1){
  			$.messager.alert('提示',"只能选择一条抽检信息进行删除",'info');
  			return;
  		}
		var row = $('#dataTabel').datagrid('getSelected');
		var checkflag = row.checkflag;
		...
		if(checkflag=="20"){
  			$.messager.alert('提示',"对不起，无法删除已经审核通过的信息！",'info');
  			return;
  		}
		...
		$.messager.confirm('提示','确定要删除吗?',function(result){
	        if (result){
	        	$.post(baseURL+'/wms/cigarettedamaged/doDel.json?inboundid='+inboundid,function(data){
	        		...
	        		$.messager.show({
						title : '提示',
						msg :  data.total+'个来抽检信息'+data.msg+'！',
					});
	        	});
	        }
	    });
...
function openCreate(){
	$('#add-fm').form('clear');
	$('#add-dlg').dialog('open').dialog('setTitle','抽检');
...
//			},
			{field:'damagedqty',title:'抽检数量(条)',width:$(this).width()*0.1,
				formatter:function(value,row,index){return row.damagedqty;}
			},
			{field:'actualqty',title:'实际抽检数量(条)',width:$(this).width()*0.1,
...
function dosave(){
	...
	if(otherqty==null||otherqty==""){
		$.messager.alert('系统提示', '请输入抽检数量(件)！', 'warning');
		return;
	}
	...
	if(actualqty==null||actualqty==""){
		$.messager.alert('系统提示', '请输入实际抽检数量(条)！', 'warning');
		return;
	}
//	if(actualqty==null||actualqty==""){
...
//	}
	$('#add-fm').form('submit',{
		...
		url:baseURL+"/wms/cigarettedamaged/doSaveabnormal.json?abid="+abid+"&damagedtype="+damagedtype,
		data:$("#add-fm").serializeArray(),
		beforeSend : function () {
			$.messager.progress({
				text : '正在新增中...',
