...
jQuery(function($){
	...
	$('#dataTabel').datagrid({
		...
		columns:[[
			...
			{field:'outtime',title:'出库日期',width:$(this).width()*0.1,sortable:true,
				formatter:function(value,row,index){return row.outtime;}
			},
...
function openCreate(){
	...
	$.ajax({
		url:baseURL+"/account/timebydm/getTimebydm.json?type=0&date="+nowTime, //type=0:根据送货日期取订单日期    type=1：根据订单日期取送货日期
	...
	var nowTimeYMDHMS = getDateYMDHMS();
	$('#outtime_new').datetimebox("setValue",nowTimeYMDHMS);
