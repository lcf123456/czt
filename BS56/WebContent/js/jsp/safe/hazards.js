...
jQuery(function($){
	...
	$('#dataTable').datagrid({
		...
		url:baseURL+"/safe/hazards/getHazards.json",  //数据来源
        collapsible:true,
         //data:data,
		queryParams:{
		...
		columns:[[
			...
			{field:'hazards',title:'危险源：作业/流程/活动/场所',width:15,sortable:false,
			...
			{field:'deptname',title:'相关部门',width:10,sortable:false,
			...
			{field:'status',title:'状态',width:10,
				formatter:function(value,row,index)
					{if( row.controllerid !=null){
						return '已控制';
					}else if (row.auditid !=null&&row.controllerid ==null){
					return '已评估';
					}
					}
			}
