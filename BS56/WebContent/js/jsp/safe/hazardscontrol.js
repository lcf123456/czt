...
jQuery(function($){
	...
	$('#dataTable').datagrid({
		...
		url:baseURL+"/safe/hazards/getHazardsControl.json",  //数据来源
        collapsible:true,
         //data:data,
		queryParams:{
		...
		columns:[[
			...
			{field:'hazards',title:'危险源：作业/流程/活动/场所',width:20,sortable:false,
			...
			{field:'effect',title:'影响后果',width:15,sortable:false,
				formatter:function(value,row,index){return row.effect;}
			},
			{field:'deptname',title:'相关部门',width:12,sortable:false,
				formatter:function(value,row,index){return row.deptname;} 
			},
			
			{field:'controlmeans',title:'控制方式',width:30,sortable:false,
				formatter:function(value,row,index){return row.controlmeans;} 
			},
