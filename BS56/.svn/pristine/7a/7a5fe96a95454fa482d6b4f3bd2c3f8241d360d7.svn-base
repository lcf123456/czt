...
jQuery(function(def){
	...
	function doReceive(){
		...
		if(rows.length==0){
			$.messager.alert('提示',"请选择要接收的退貨信息",'info');
			return;
		}
		$.messager.confirm('提示','确定要接收所选退貨信息吗?',function(result){
			if (result){
				var ids = "";
				...
				$.post(baseURL+'/account/operate/doReceive.json'+ids,function(data){
					$('#dataTable').datagrid('reload'); 
					//console.log("del--"+data);
					data = eval('('+data+')');
					$.messager.show({
						title : '提示',
						msg :  data.total+'个退貨信息接收'+data.msg+'！',
					});
				});
			}
		})
