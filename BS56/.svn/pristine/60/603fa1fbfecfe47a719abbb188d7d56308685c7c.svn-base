...
<script type="text/javascript" src="<spring:url value="/js/jsp/perform/deptmonthsumdeptmngadd.js"/>"></script>
  <head>
  ...
  var obid=0;//主表id,用于标识是否为第一次提交 0为第一次，否则是修改
  ...
  <body>
	...
	<table id="worklist" class="easyui-datagrid" title="考核列表" "
            data-options="
                iconCls: 'icon-edit',
                singleSelect: true,
                toolbar: '#dlg-buttons',
                url:baseURL+'/perform/keywork/getDeptmngAllKeyworkList.json?searchdept='+deptid+'&searchtime='+scoremonth, 
                method: 'get',
                height:380,
            ...
            //合并单元格  
                 ...
                 var hj  = compute('scorevalue');
		    $('#worklist').datagrid('updateRow',{index:$('#worklist').datagrid('getRows').length-1,row:{scorevalue:hj}});
                hj  = compute('selfscore');
        ...
        <thead>
            <tr>
                ...
                <th data-options="field:'createname',width:$(this).width()*0.05,align:'right'">创建人</th>
                <th data-options="field:'finishnote',width:$(this).width()*0.2,align:'right',editor:'textbox'">完成情况</th>
                <th data-options="field:'selfscore',width:$(this).width()*0.05,align:'right',editor:{type:'numberbox',options:{precision:2,max:100,min:0}}">评分</th>
    ...
    <input type="hidden" name="allrows" id="allrows" class="easyui-textbox"></input> <!--保存所有行的数据 -->
			...
			<input type="hidden" name="obid" id="obid" class="easyui-textbox"></input> <!-- 主表id,用于标识是否为第一次提交 0为第一次，否则是修改 -->
    		<input type="hidden" name="ctype" id="ctype" class="easyui-textbox"></input> <!-- 10：副部长考核  20：部长考核 -->
    		<input type="hidden" name="tmpscore" id="tmpscore" class="easyui-textbox"></input> <!-- 未加入考核的分数 -->
		</form>
	 ...
	 <div id="dlg-buttons">
		...
		<a href="#" class="easyui-linkbutton" id="btn-done" iconCls="icon-cancel" onclick="closethistab()">关闭</a>
	</div>
	</div>
...
//		  }
		  ...
		  function tosave(){
			  ...
			  var rows = $('#worklist').datagrid('getRows');
			    var entities = "";
			    for(i = 0;i < rows.length-1;i++){//比提交合计
			    	 entities = entities  + JSON.stringify(rows[i]);  
			    }
			    entities = "["+entities+"]";
			    console.log("updaterows-6--"+entities);
			    $('#allrows').textbox("setValue",entities);//所有行记录
				   ...
				   var hj  = compute("selfscore");//$('#worklist').datagrid('getRows')
					...
					$('#obid').textbox("setValue",obid);//主表id,用于标识是否为第一次提交 0为第一次，否则是修改
					$('#obid').textbox("setText",obid);
					$('#ctype').textbox("setValue","20");//10：副部长考核  20：部长考核
					$('#ctype').textbox("setText","20");
					$('#tmpscore').textbox("setValue",tmpscore);// 未加入考核的分数 
					$('#tmpscore').textbox("setText",tmpscore);
					 $('#add-fm').form('submit',{
							onSubmit: function(){
								var isValidate = $(this).form('validate');
								if(isValidate){
