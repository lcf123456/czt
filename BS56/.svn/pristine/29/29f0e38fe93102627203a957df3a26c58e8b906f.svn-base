...
<script type="text/javascript" src="<spring:url value="/js/jsp/perform/deptmonthsumedit.js"/>"></script>
  <head>
  ...
  <body>
	...
	<div id="add-dlg" >
		<form  id="add-fm" method="post" action="" novalidate  >
			...
			<tr>
			 ...
			 <div style="WIDTH: 100%; HEIGHT: auto; BACKGROUND-COLOR: transparent; OVERFLOW: scroll; scrollbar-face-color:#c1d9f5; scrollbar-arrow-color:#ffffff; scrollbar-highlight-color:#ffffff; scrollbar-3dlight-color:#70807d; scrollbar-shadow-color:#ffffff; scrollbar-darkshadow-color:#70807d; scrollbar-track-color:#ffffff">
	<table id="worklist" class="easyui-datagrid" title="考核列表" style="width:100%;height:auto"
        ...
        <thead>
            <tr>
                ...
                <th data-options="field:'otherscore',width:$(this).width()*0.05,editor:{type:'numberbox',options:{precision:2,max:100,min:0}}">部长评分</th>
                <th data-options="field:'note',width:$(this).width()*0.1,editor:'textbox'">加扣分说明</th>
            </tr>
        </thead>
    </table>
    </div>
    <input type="hidden" name="allrows" id="allrows" class="easyui-textbox"></input> <!--保存所有行的数据 -->
	 ...
	 <div id="dlg-buttons">
		...
		<a href="#" class="easyui-linkbutton" id="btn-done" iconCls="icon-cancel" onclick="closethistab('部长评分')">关闭</a>
	</div>
	</div>
...
//		  }
		  ...
		  function tosave(){
			  ...
			  var rows = $('#worklist').datagrid('getRows');
			    var entities = "";
			    for(i = 0;i < rows.length-1;i++){//不提交合计
				   ...
				   var hj  = compute("otherscore");//$('#worklist').datagrid('getRows')
					...
					$('#obid').textbox("setText",obid);
					 $('#add-fm').form('submit',{
							...
							url:baseURL+"/perform/deptmonthsum/doAuditOne.json?obid="+obid,
							//data: {'},
							beforeSend : function () {
								$.messager.progress({
									text : '正在新增中...',
