...
function openCreate(){
	$('#add-fm').form('clear');
	$('#add-dlg').dialog('open').dialog('setTitle','散烟区至分拣区移库');
	...
	$('#targetareaname_new').textbox("setValue","分拣区");
	$('#targetareaname_new').textbox("setText","分拣区");
		//初始化货主
...
{
	//分拣区信息选择清空：
	$('#cigarettecode_secondok').textbox("setValue","");
	$('#cigarettecode_secondok').textbox("setText","");
	$('#cigarettename_secondok').textbox("setValue","");
	$('#cigarettename_secondok').textbox("setText","");
	$('#cigarettetypename_secondok').textbox("setValue","");
	$('#cigarettetypename_secondok').textbox("setText","");
	$('#troughnum_secondok').textbox("setValue","");
	$('#troughnum_secondok').textbox("setText","");
	$('#machineseq_secondok').textbox("setValue","");
	$('#machineseq_secondok').textbox("setText","");
	$('#groupno_secondok').textbox("setValue","");
	$('#groupno_secondok').textbox("setText","");
		...
		$.ajax({ 
		    ...
		    success: function(data){
		    	...
		    	if(parseFloat(qtynew) > parseFloat(actualqty) || parseFloat(actualqty)==0 || isNaN(parseFloat(qtynew))) {
		    		...
		    		$('#cigarettecode_firstok').textbox("setValue",cigarettecode);
					$('#cigarettecode_firstok').textbox("setText",cigarettecode);
					$('#cigarettename_firstok').textbox("setValue",cigarettename);
					$('#cigarettename_firstok').textbox("setText",cigarettename);
					$('#jtsize_firstok').textbox("setValue",jtsize);
					$('#jtsize_firstok').textbox("setText",jtsize);
					$('#qtynew_firstok').textbox("setValue",qtynew);
					$('#qtynew_firstok').textbox("setText",qtynew);
		    		$("#fisstdiv" ).css("display", "none"); //卷烟列表不显示
		    		$("#seconddiv" ).css("display", "block"); //分拣通道列表显示
		    		
		    		//根据卷烟code初始分拣通道数据列表
		    		initroughist(cigarettecode);
		    		//入库单详情
...
function toSave()
{
	var cigarettecode = $("#cigarettecode_secondok").val();
	var troughnum = $("#troughnum_secondok").val();//通道号
	var groupno = $("#groupno_secondok").val();//组次
	var qtynew = $("#qtynew_firstok").val();
	var barcode = $("#barcode").val();

	  if(cigarettecode==""){
		  $.messager.alert('提示',"移库卷烟选择出错，请重新选择！",'info');
			return;
	  }
	  if(troughnum==""){
		  $.messager.alert('提示',"请选择接收的分拣通道信息！",'info');
			return;
	  }
	  if(qtynew==""){
		  $.messager.alert('提示',"移库卷烟数量出错，请重新输入！",'info');
			return;
	  }
	  if(barcode==""){
		  $.messager.alert('提示',"件码出错，请重新选择！",'info');
			return;
	  }
	  $('#add-fm').form('submit',{
			onSubmit: function(){
			...
			url:baseURL+"/wms/moveareastock/doSave.json?movetype=10&targetareaid="+targetareaid+"&areaid="+areaid+"&obid="+obid, 
			success : function(data) {
				data = eval('('+data+')');
	   ...
	   */
	    ...
	    function doAudit(checkflag){
	    	...
	    	$.messager.confirm('提示',alertMsg,function(result){
	    		if(result){
	    			$('#add-fm').form('submit',{
	        			url:baseURL+"/wms/moveareastock/doAudit.json?id="+obid+"&checkdescribe="+checkdescribe+"&checkflag="+checkflag+"&movetype="+movetype+"&operationType=10", 
