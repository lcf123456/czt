...
@Autowired
	...
	@Override
	public List<GroupinfoVo> useGroupinfoVoList(GroupinfoVo groupinfoVo) {
		// TODO 自动生成的方法存根
		return this.GroupinfoVoMapper.useGroupinfoVoList(groupinfoVo);
	}
	...
	public int delGroupuserVo(List<Integer> ids) {
		if(ids != null && ids.size() > 0) {
			for (Integer id : ids) {
				this.GroupinfoVoMapper.deleteByUserPrimaryKey(id);
			}
			return ids.size();
		}
		return 0;
