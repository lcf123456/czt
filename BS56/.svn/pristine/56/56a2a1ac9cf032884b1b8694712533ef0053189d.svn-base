...
jQuery(function($){
	...
	$('#dataTabel').datagrid({
		...
		nowrap:false,
		//货主默认为长沙11430101
		url:baseURL+"/perform/userperformlevel/getUserperformlevelPageList.json?authority=general", //ctype=10:重点工作
		sortName: 'id', //排序的列
		sortOrder: 'desc', //倒序
		...
		columns:[[
			...
			{field:'username',title:'用户',width:$(this).width()*0.05,
				formatter:function(value,row,index){return row.username;}
			},
			{field:'deptname',title:'部门',width:$(this).width()*0.07,
				formatter:function(value,row,index){return row.deptname;}
			},
			{field:'checkdatestr',title:'评定月份',width:$(this).width()*0.1,
				formatter:function(value,row,index){return row.checkdatestr;}
			},
			{field:'levelname',title:'等级',width:$(this).width()*0.1,
				formatter:function(value,row,index){return row.levelname;}
			},
			{field:'score',title:'分值',width:$(this).width()*0.1,
				formatter:function(value,row,index){return row.score;}
			},
			{field:'appraisername',title:'评定人',width:$(this).width()*0.1,
				formatter:function(value,row,index){return row.appraisername;}
			}
		]],
		toolbar:'#toolbar',
...
function openCreate(){
	$('#add-fm').form('clear');
	$('#add-dlg').dialog('open').dialog('setTitle','员工考核新增');
	
	//初始化记录人、记录日期、检查日期、检查部门
	var obj = $("#scoremonth_new");
	initDataboxYM(obj);
...
function getkwlist(obj,dgheight)
{
	...
	obj.datagrid({
		...
		nowrap:false,
		url:baseURL+"/perform/keywork/getKeyworkList.json?ctype=10", //ctype=10:重点工作
		sortName: 'id', //排序的列
		sortOrder: 'desc', //倒序
		remoteSort: true, //服务器端排序
		idField:'id', //主键字段
	...
	//删除
		function deleterow(){
	        ...
	        if (result){
	        	...
	        	$.post(baseURL+'/perform/userperformlevel/doDel.json?id='+id,function(data){
		        	$('#dataTabel').datagrid('reload'); 
	        		$.messager.show({
						title : '提示',
